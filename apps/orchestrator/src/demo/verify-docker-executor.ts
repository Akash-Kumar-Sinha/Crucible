...
async function verifyDockerExecution() {
  ...
  console.log(`[1] Docker Daemon Availability: ${available ? "✅ CONNECTED" : "❌ UNREACHABLE"}`);
  if (!available) {
  ...
  console.log("Launching tool command: 'node -e \"setTimeout(() => console.log(\\\"container_output_verified\\\"), 2000)\"'...");

  const execPromise = dockerExecutor.execute({
    command: 'node -e "setTimeout(() => console.log(\\"container_output_verified\\"), 2000)"',
    language: "node",
    toolName: "bash_exec",
  ...
  const psOutput = execSync('docker ps --filter "label=crucible.managed=true" --format "table {{.ID}}\t{{.Image}}\t{{.Status}}\t{{.Names}}"').toString().trim();
  ...
  const psAfter = execSync('docker ps --filter "label=crucible.managed=true" --format "{{.ID}}"').toString().trim();
  console.log(`[Teardown Check]: Active Crucible containers after completion: ${psAfter ? psAfter : "0 (Cleanly Removed ✅)"}`);

  if (result1.exitCode !== 0 || !result1.stdout.includes("container_output_verified") || psAfter !== "") {
    throw new Error("Test 1 Failed: Container output was incorrect or container was not cleaned up.");
  }
  console.log("✅ TEST 1 PASSED: Container spun up, executed, captured output, and tore down.");
  ...
  if (pyResult.stdout !== "5050" || pyResult.exitCode !== 0) {
    throw new Error(`Test 2 Failed: Python output expected '5050' but got '${pyResult.stdout}'`);
  }
  console.log("✅ TEST 2 PASSED: Output correctly captured from container sandbox.");
  ...
  errorReporter.once("containerFailure", (record) => {
    ...
    console.log(`   Level: ${record.level}, Container: ${record.containerContext?.containerId}`);
  });
  ...
  const runningContainerId = execSync('docker ps -q --filter "label=crucible.managed=true"').toString().trim().split("\n")[0];
  console.log(`Found running container ID: ${runningContainerId}. Issuing 'docker kill' now...`);
  execSync(`docker kill ${runningContainerId}`);
  console.log("Container killed externally.");

  const killResult = await killExecPromise;
  console.log(`Execution returned without hanging! Exit code: ${killResult.exitCode}, Duration: ${killResult.durationMs}ms`);
  console.log(`ErrorReporter captured failure: ${capturedFailureReason ? "YES ✅" : "NO ❌"}`);

  if (killResult.exitCode === 0) {
    throw new Error("Test 3 Failed: Killed container unexpectedly reported exit code 0.");
  }
  console.log("✅ TEST 3 PASSED: Killed container surfaced cleanly as logged error without hanging.");
