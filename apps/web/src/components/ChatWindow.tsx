...
export function ChatWindow({
  ...
  if (!session) {
    return (
      <main className="flex-1 flex flex-col justify-between bg-zinc-900 h-screen p-6 sm:p-10 relative overflow-hidden">
            ...
            <QuestionnaireItem name="starter-templates">
              <QuestionnaireChoices className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <QuestionnaireChoice
                  ...
                  className="rounded-xl border border-white/8 hover:border-white/20 bg-zinc-800/40 hover:bg-zinc-800/70 p-4 transition-all group"
                >
  ...
  return (
    <main className="flex-1 flex flex-col h-screen bg-zinc-900 overflow-hidden relative">
      {/* Session Topbar */}
      <header className="h-14 border-b border-white/8 flex items-center justify-between px-4 sm:px-6 bg-zinc-900/90 backdrop-blur-md z-10">
        <div className="flex items-center gap-3 min-w-0">
          <div className="flex items-center gap-2 min-w-0">
            <h1 className="text-sm font-semibold text-white truncate max-w-[200px] sm:max-w-sm">
          ...
          <JobStatusBadge
            phase={
              effectiveStatus === "queued"
                ...
                : effectiveStatus === "running"
                  ? infraData?.kubernetes?.job?.phase || "Running"
            ...
            jobName={infraData?.kubernetes?.job?.jobName}
            podName={infraData?.kubernetes?.job?.podName}
            nodeName={infraData?.kubernetes?.job?.nodeName}
            durationMs={infraData?.kubernetes?.job?.durationMs}
          />

          {/* Queue position badge when run is waiting in backlog */}
          <QueuePositionBadge
            position={infraData?.queue?.position ?? -1}
            estimatedWaitMs={infraData?.queue?.estimatedWaitMs ?? 0}
            backlogCount={
              infraData?.queue?.backlogCount ?? infraData?.queue?.backlog ?? 0
            }
          />
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <Radio
              size={10}
              className={isStreamConnected ? "animate-pulse" : ""}
            />
            <span className="hidden sm:inline">
              {isStreamConnected ? "Live Stream" : "Reconnecting"}
            </span>
          </div>

          {/* Sandbox Info Trigger Button */}
          <Button
            type="button"
            variant="outline"
            size="sm"
            onClick={() => setShowSandboxInfo(true)}
            className="h-7 text-xs bg-zinc-800/60 border-white/8 hover:bg-zinc-800 text-zinc-300 hover:text-white"
          >
      ...
      {/* Bottom Input Area */}
      <div className="border-t border-white/8 bg-zinc-900/90 px-4 sm:px-8 py-4 backdrop-blur-xl">
        <PromptInput
          value={input}
          onChange={setInput}
          onSubmit={() => {
