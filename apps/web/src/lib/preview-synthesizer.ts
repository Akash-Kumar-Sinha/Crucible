/**
 * Client-Side Preview Synthesizer: Converts assistant message code blocks
 * into fully-functional, interactive React/HTML/Tailwind live preview documents.
 */
...
export function synthesizeLivePreviewFromMessages(
  messages: Array<{ role: string; content?: string; toolCalls?: any[] }>,
  sessionId?: string,
): string | null {
  if (!messages || messages.length === 0) return null;

  // Search in reverse from most recent assistant message
  for (let i = messages.length - 1; i >= 0; i--) {
    const msg = messages[i];
    if (msg.role === "assistant" && msg.content) {
      const synthesized = synthesizePreviewDoc(msg.content, sessionId);
      if (synthesized) return synthesized;
    }

    // Check tool call outputs or arguments (e.g. write_file)
    if (msg.toolCalls) {
      for (const tc of msg.toolCalls) {
        if (tc.name === "write_file" && tc.arguments?.content) {
          const synthesized = synthesizePreviewDoc(
...
export function synthesizePreviewDoc(
  content: string,
  sessionId?: string,
): string | null {
